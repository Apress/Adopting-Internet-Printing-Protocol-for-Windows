...
namespace IppPrintRequest
{
    public class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            Console.WriteLine("Would you like to getattributes, validate, create, cancel, senddoc, getjobs, identify, or printjob?");
            ...
            else if (sAction == "SENDDOC")
            {
                ...
                try
                {
                    ...
                    if (await CheckFileTypeForSupport(sIppPrinter, request, file) == false)
                    {
                        Console.WriteLine("File type not supported by printer");
                        return;
                    }
            ...
            else if (sAction == "GETATTRIBUTES")
            {
                ...
                try
                {
                    GetPrinterAttributesRequest gpa = new GetPrinterAttributesRequest("1.1", sIppPrinter, false, request);
            ...
            Console.WriteLine("--IPP request completed--");
        }

        /// <summary>
        /// CheckFileTypeForSupport
        /// 
        /// Checks to see that the file extension is supported on the target printer.
        /// 
        ...
        /// <returns></returns>
        static async Task<bool> CheckFileTypeForSupport(string sPrinter, int request,string sFile)
        {
            GetPrinterAttributesRequest gpa = new GetPrinterAttributesRequest("1.1", sPrinter, false, request);
            CompletionStruct cs = await gpa.SendRequestAsync();
            string type = gpa.GetMimeType(sFile);
            return gpa.IsSupported("document-format-supported", type);
        }    
    }
}
