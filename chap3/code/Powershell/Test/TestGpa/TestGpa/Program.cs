...
namespace TestGpa
{
    internal class Program
    {
        ...
        {
            string sAttributes = "printer-description";
            string[] req_attributes = sAttributes.Split(',');
            await MakeRequest("HPM528", req_attributes);
        }
        static  async Task MakeRequest(string printer_name, string[] requested_attributes)
        {
        ...
        static async Task GetIppResponse(HttpResponseMessage response)
        {
            try
            {
                ...
                {
                    using (var reader = new BinaryReader(responseStream, Encoding.UTF8, true))
                    {
                        ...
                        Console.WriteLine($"The version returned was {MajorByte}.{MinorByte}");
                        ...
                        Console.WriteLine($"The request Id was: {StatusCode}");                        //Look at the next 4 bytes
                        int RequestId = ByteOrder.Flip(reader.ReadInt32());
                        Console.WriteLine($"The request Id was: {RequestId}");
        ...
        static void GetIppAttributes(BinaryReader reader)
        {
        ...
        /// <returns></returns>
        /// <exception cref="Exception"></exception>
        static string GetAttribute(ValueTag tag, BinaryReader stream, string previous)
        {
        ...
        /// <returns></returns>
        /// <exception cref="Exception"></exception>
        static object GetValueTagValue(BinaryReader stream, ValueTag vt)
        {
            switch (vt)
            {
