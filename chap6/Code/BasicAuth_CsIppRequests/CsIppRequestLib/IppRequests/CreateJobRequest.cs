...
{
    public class CreateJobRequest: IppRequest
    {
        private readonly byte[] m_bIppVersionAndOperationId = new byte[] { 0x01, 0x01, 0x00, (byte)RequestType.CREATE_JOB }; // IPP version and operation ID
        private readonly object _lock = new object();

        public CreateJobRequest(string ipp_version, string printerUri, bool encrypted, int requestId, Dictionary<string, List<object>> jobAttributes) : base(printerUri, encrypted, requestId)
        {
            m_bChunkRequest = false;
        ...
        public override async Task<CompletionStruct> SendRequestAsync()    
        {
            try
            {
                ...
                return await SendIppRequestWithAuthAsync(RequestType.CREATE_JOB, provider);
            }
            catch (Exception ex)
            {
