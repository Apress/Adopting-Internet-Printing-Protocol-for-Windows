...
namespace CS_Multi_Check
{
    public static class WinIppPinvokeMethods
    {
        ...
        [DllImport("WinIpp.dll", EntryPoint = "getIppErrorString", SetLastError = true, CallingConvention = CallingConvention.Cdecl)]
        ...
        /// into list of strings which are then passed to the caller. This version
        /// differs only in that it has an awaiter in the return Task<List<string>>.
        /// 
        /// 
        ...
        /// <returns></returns>
        /// 
        public static async Task<List<string>> _getPrinterAttributes(string hostname, string user_name, uint encryption)
        {
            int size = 0;
            int retCode = 0;
            StringBuilder sb = new StringBuilder(hostname);
            ...
            try
            {
                IntPtr ptr = await Task.Run(() => GetPrinterAttributes(sb, sbun, ref size, ref retCode, (uint)encryption));
                if (retCode == Defines.IPP_STATUS_OK)
                {
                    lstStrings.Add("====================|");
                    lstStrings.Add("Printer|" + hostname);
                    for (int i = 0; i < size; i++)
                    {
                        IntPtr strPtr = Marshal.ReadIntPtr(ptr, i * IntPtr.Size);
                        lstStrings.Add(Marshal.PtrToStringAnsi(strPtr));
                    }
                }
                else
                {
