...
namespace CsIppRequestLib
{
    public abstract class IppRequest
    {
        ...
        private void MakeOperationAttributes(string printerUri)
        {
            ...
            m_bAttributesCharset = RequestHelpers.CreateIppAttribute(0x47, "attributes-charset", "utf-8");
            m_bAttributesNaturalLanguage = RequestHelpers.CreateIppAttribute(0x48, "attributes-natural-language", "en");
            m_bPrinterUriAttribute = RequestHelpers.CreateIppAttribute(0x45, "printer-uri", printerUri);
            m_bRequestingUserName = RequestHelpers.CreateIppAttribute(0x42, "requesting-user_name", Environment.UserName);
        }
        ...
        protected async Task<CompletionStruct> SendIppRequestAsync(RequestType type)
        {
            ...
            using (var cts = new CancellationTokenSource(TimeSpan.FromSeconds(30)))
            {
                ...
                try
                {
                    ...
                    return await GetResponseAsync(type, response);
                }
        ...
        protected async Task<CompletionStruct> GetResponseAsync(RequestType type, HttpResponseMessage response)
        {
        ...
        protected async Task<CompletionStruct> SendIppRequestWithAuthAsync(RequestType type, ICredentialsProvider credentialsProvider)
        {
            ...
            while (!isAuthenticated)
            {
                using (var cts = new CancellationTokenSource(TimeSpan.FromSeconds(30)))
                {
                    // IMPORTANT: Create a new HttpContent instance each time because the original may be disposed.
                    HttpContent content;
                    if (m_bChunkRequest)
                    {
                        var stream = new MemoryStream(m_bRequestPayload);
