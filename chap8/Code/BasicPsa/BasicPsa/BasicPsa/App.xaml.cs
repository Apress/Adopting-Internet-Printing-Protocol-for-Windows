    ...
    protected override void OnActivated(IActivatedEventArgs args)
    {
        if (args.Kind == ActivationKind.PrintSupportSettingsUI)
        {
            ...
            if (settingsEventArgs != null)
            {
                ...
                string sDocName = settingsEventArgs.Session.DocumentTitle;
                string sPrintername = settingsEventArgs.Session.SessionInfo.Printer.PrinterName;
                if (rootFrame != null)
                {
                    switch (settingsSession.LaunchKind)
                    {
                        case SettingsLaunchKind.UserDefaultPrintTicket:
                            {
                                rootFrame.Navigate(typeof(DefaultSettingView), settingsSession);
                            }
                            break;
                        case SettingsLaunchKind.JobPrintTicket:
                            {
                                //Display standard Print Dialog/Settings and (when "More Settings" clicked), page with Special HP Attributes
                                rootFrame.Navigate(typeof(JobPrintTicketView), settingsSession);
                            }
                            break;
                    }
