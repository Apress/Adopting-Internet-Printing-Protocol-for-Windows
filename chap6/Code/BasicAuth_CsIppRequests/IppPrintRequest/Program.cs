...
using static CsIppRequestLib.Status;
...
namespace IppPrintRequest
{
    public class Program
    {
        static async Task Main(string[] args)
        {
            ...
            Console.WriteLine("Would you like to getattributes, getjobattributes, validate, create, cancel, senddoc, getjobs, identify, pauseprinter, resumeprinter, or printjob?");
            ...
            if (sAction == "VALIDATE")
            {
                ...
                {
                    Console.WriteLine(Status.GetIppStatusFromException(ex));
            ...
            else if (sAction == "CREATE")
            {
                ...
                try
                {
                    ...
                    if (cs.status <= (int)Status.IPP_STATUS.IPP_STATUS_OK_EVENTS_COMPLETE)
                    {
                        ...
                        Console.WriteLine($"Job creation failed, status returned was {cs.status}");
            ...
            else if (sAction == "SENDDOC")
            {
                ...
                try
                {
                    ...
                    if (cs.status <= (int)Status.IPP_STATUS.IPP_STATUS_OK_EVENTS_COMPLETE)
                    {
                        Console.Write("SendDoc request was successful...");
                    }
                    else
                    {
                        Console.WriteLine(Status.GetIppStatusMessage(cs.status));
            ...
            else if (sAction == "CANCEL")
            {
                ...
                try
                {
                    ...
                    if (cs.status <= (int)Status.IPP_STATUS.IPP_STATUS_OK_EVENTS_COMPLETE)
                    {
                        Console.WriteLine("Print job successfully cancelled.");
                    }
                    else
                    {
                        Console.WriteLine(Status.GetIppStatusMessage(cs.status));
            ...
            else if (sAction == "IDENTIFY")
            {
                ...
                try
                {
                    ...
                    if (cs.status <= (int)Status.IPP_STATUS.IPP_STATUS_OK_EVENTS_COMPLETE)
                    {
                        Console.WriteLine("Printer successfully identified");
                    }
                    else
                    {
                        Console.WriteLine(Status.GetIppStatusMessage(cs.status));
            ...
            else if (sAction == "PAUSEPRINTER")
            {
                //----------------------Pause-Printer Request------------------------
                try
                {
                    PausePrinterRequest ppr = new PausePrinterRequest("1.1", sIppPrinter, false, request);
                    CompletionStruct cs = await ppr.SendRequestAsync();
                    if (cs.status <= (int)Status.IPP_STATUS.IPP_STATUS_OK_EVENTS_COMPLETE)
                    {
                        Console.WriteLine("Printer successfully paused operation");
                    }
                    else
                    {
                        Console.WriteLine(Status.GetIppStatusMessage(cs.status));
            ...
            else if (sAction == "RESUMEPRINTER")
            {
                //----------------------Resume-Printer Request------------------------
                try
                {
                    ResumePrinterRequest rpr = new ResumePrinterRequest("1.1", sIppPrinter, false, request);
                    CompletionStruct cs = await rpr.SendRequestAsync();
                    if (cs.status <= (int)Status.IPP_STATUS.IPP_STATUS_OK_EVENTS_COMPLETE)
                    {
                        Console.WriteLine("Printer successfully resumed operation");
                    }
                    else
                    {
                        Console.WriteLine(Status.GetIppStatusMessage(cs.status));
            ...
            else if (sAction == "GETATTRIBUTES")
            {
                ...
                try
                {
                    GetIppAttributesRequest gpa = new GetIppAttributesRequest("1.1", sIppPrinter, false, request);
            ...
            else if (sAction == "GETJOBATTRIBUTES")
            {
                //----------------------Get Printed Job Attributes Request------------------------
                Console.WriteLine("Enter the job id to query: ");
                int jobId = Int32.Parse(Console.ReadLine());
                try
                {
                    GetJobAttributesRequest gja = new GetJobAttributesRequest("1.1", sIppPrinter, false, request, jobId);
                    CompletionStruct cs = await gja.SendRequestAsync();
                    Console.WriteLine(Status.GetIppStatusMessage(cs.status));
                }
                catch (Exception ex)
                {
            ...
            else if (sAction == "GETJOBS")
            {
                try
                {
                    ...
                    Console.WriteLine("Do you want completed jobs only?  T/F ");
