...
namespace GetPrinterAttributesRequest
{
    public class ResponseHelpers
    {
        ...
        public static void GetIppAttributes(BinaryReader reader, IppPrinters _printers, IppPrinter ipp_printer)
        {
            ...
            do
            {
                ...
                {    
                    ...
                    case DelimiterTag.EndOfAttributesTag:
                        _printers.AddIppPrinter(ipp_printer);
                        return;
                    default:
                        //print out attribute name and value(s)
                        var attribute = GetAttribute((ValueTag)b, reader, prevAttribute, ipp_printer);
        ...
        public static string GetAttribute(ValueTag value_tag, BinaryReader stream, string previous, IppPrinter ipp_printer)
        {
            var len = ByteOrder.Flip(stream.ReadInt16());
            var name = Encoding.ASCII.GetString(stream.ReadBytes(len));
            var value = GetValueTagValue(stream, value_tag);
            ...
            ipp_printer.AddNewAttribute(name, value.ToString(), (byte)value_tag);
