...
{
    public class ResumePrinterRequest : IppRequest
    {
        private readonly byte[] m_bIppVersionAndOperationId = new byte[] { 0x01, 0x01, 0x00, (byte)RequestType.RESUME_PRINTER }; // IPP version and operation ID
        private readonly object _lock = new object();

        public ResumePrinterRequest(string ipp_version, string printerUri, bool encrypted, int requestId) : base(printerUri, encrypted, requestId)
        {
        ...
        public override async Task<CompletionStruct> SendRequestAsync()
        {
            try
            {
                ...
                return await SendIppRequestWithAuthAsync(RequestType.RESUME_PRINTER, provider);
            }
            catch (Exception ex)
            {
