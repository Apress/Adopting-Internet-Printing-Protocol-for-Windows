...
{
    public class PausePrinterRequest : IppRequest
    {
        private readonly byte[] m_bIppVersionAndOperationId = new byte[] { 0x01, 0x01, 0x00, (byte)RequestType.PAUSE_PRINTER }; // IPP version and operation ID
        private readonly object _lock = new object();

        public PausePrinterRequest(string ipp_version, string printerUri, bool encrypted, int requestId) : base(printerUri, encrypted, requestId)
        {
        ...
        public override async Task<CompletionStruct> SendRequestAsync()
        {
            try
            {
                ...
                return await SendIppRequestWithAuthAsync(RequestType.PAUSE_PRINTER, provider);
            }
            catch (Exception ex)
            {
