...
namespace CS_GPA_CH3_WPF
{
    public static class WinIppPinvokeMethods
    {
        ...
        public static async Task<PrinterAttributes> _getPrinterAttributes(string hostname, string user_name, uint encryption)
        {
            int size = 0;
            int retCode = 0;
            StringBuilder sb = new StringBuilder(hostname);
            ...
            try
            {
                ...
                if (retCode == Defines.IPP_STATUS_OK)
                {
                    ...
                    for (int i = 0; i < size; i++)
                    {
                        ...
                        _pas.AddAttribute(Marshal.PtrToStringAnsi(strPtr));
                    }
                }
                else
                {
